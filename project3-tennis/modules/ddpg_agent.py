...
class Agent:
    ...
    def __init__(self,
        ...
        Params
        ======
            number_of_agents (int):     number of agents that are needed to solve the task
            ...
            batch_size (int):           mini batch size to train the neural network with
            update_every (int):         number after how many episodes we train the networks again
        ...
        # Replay memory
        self._memory = ReplayBuffer(action_size,
        ...
        for i in range(0, number_of_agents):
            self._agents.append(
                HelperAgent(agent_name="agent" + str(i),
    ...
    def for_training(cls,
        ...
        return cls(number_of_agents, state_size, action_size, lr_actor, lr_critic, weight_decay_critic,
                   replay_buffer_size, batch_size, update_every, gamma, tau, seed, device,
                   is_training=True)

    @classmethod
    def for_playing(cls, number_of_agents, state_size, action_size, prefix, seed=0):
...
class HelperAgent:
    ...
    def save_model(self, prefix):
        torch.save(self._actor_local.state_dict(), prefix + self._agent_name + '_actor_local.pth')
        torch.save(self._actor_target.state_dict(), prefix + self._agent_name + '_actor_target.pth')

        torch.save(self._critic_local.state_dict(), prefix + self._agent_name + '_critic_local.pth')
        torch.save(self._critic_target.state_dict(), prefix + self._agent_name + '_critic_target.pth')

    def load(self, prefix):
        self._actor_local.load_state_dict(torch.load(prefix + self._agent_name + '_actor_local.pth'))
        self._actor_target.load_state_dict(torch.load(prefix + self._agent_name + '_actor_target.pth'))

        self._critic_local.load_state_dict(torch.load(prefix + self._agent_name + '_critic_local.pth'))
        self._critic_target.load_state_dict(torch.load(prefix + self._agent_name + '_critic_target.pth'))
